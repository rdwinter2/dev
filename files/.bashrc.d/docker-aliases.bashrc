...
alias dkrmiorphans='docker rmi $(docker ps -q) '
...
dkpull() {
  docker pull docker.$(dnsdomainname)/$1
}
dktag() {
  docker tag $(docker images docker.$(dnsdomainname)/$1 -q) $1
  docker tag $(docker images docker.$(dnsdomainname)/$1 -q) docker.internal/$1
}
dkpush() {
  docker push docker.internal/$1
}
